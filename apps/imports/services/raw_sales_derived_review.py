...
class RawSalesDerivedReviewError(Exception):
    def __init__(
        self,
        code: str,
        message: str,
...
class RawSalesDerivedReviewResult:
    source_upload: ImportSourceUpload
    batch: ImportBatch


def _logical_sales_scope_batches(
    *,
...
def _resolve_sales_brand_code(
    review: RawSalesReviewResult,
) -> str:
    if not review.adapted.rows:
        raise RawSalesDerivedReviewError(
            "empty_sales_file",
            "The raw Sales file contains no sales rows.",
        )
    ...
    if len(vans) != 1:
        raise RawSalesDerivedReviewError(
            "multiple_sales_trucks",
            (
                "A raw Sales file must resolve to "
    ...
    if (
        ...
        raise RawSalesDerivedReviewError(
            "missing_van",
            "The adapted Sales file has no VAN.",
            details={
                "van": van,
            },
        )
    ...
    if (
        ...
        raise RawSalesDerivedReviewError(
            "truck_not_found",
            (
                "The adapted Sales VAN does not match "
                "a DELISKY BI truck."
            ),
            details={
                "van": van,
    ...
    if (
        ...
        raise RawSalesDerivedReviewError(
            ...
            (
                "The adapted Sales VAN matches more "
    ...
    if truck is None:
        raise RawSalesDerivedReviewError(
            ...
            "The Sales truck resolution result is invalid.",
            details={
                "van": van,
            },
        )
    ...
    if brand is None:
        raise RawSalesDerivedReviewError(
            ...
            (
                "The resolved Sales truck has no "
                "distribution brand."
            ),
            details={
                "van": van,
    ...
    if not brand_code:
        raise RawSalesDerivedReviewError(
            ...
            (
                "The resolved Sales truck distribution "
                "brand has no code."
            ),
            details={
                "van": van,
...
def create_raw_sales_derived_import_review(
    source: Any,
    *,
    source_system_code: str,
    uploaded_by: Any,
    ...
    review = prepare_raw_sales_review(
        source,
    ...
    brand_code = _resolve_sales_brand_code(
        review
    )
    ...
    return RawSalesDerivedReviewResult(
        source_upload=source_upload,
        batch=batch,
    )
