...
from apps.imports.models import ImportSourceSystem, ImportSourceUpload
...
class RawItemsDetailMultiBrandReviewTests(TestCase):
    ...
    def test_persists_every_brand_partition_in_review_order(self, persist_brand):
        bifa = SimpleNamespace(brand_code="BIFA")
        delisky = SimpleNamespace(brand_code="DELISKY")
        first_result = SimpleNamespace(batch=SimpleNamespace(pk=101))
        second_result = SimpleNamespace(batch=SimpleNamespace(pk=102))
        persist_brand.side_effect = [first_result, second_result]
        ...
        self.assertEqual(result.brand_results, (first_result, second_result))
        self.assertEqual(
            tuple(batch.pk for batch in result.batches),
            (101, 102),
        )
        self.assertEqual(persist_brand.call_count, 2)
        self.assertIs(
            persist_brand.call_args_list[0].kwargs["brand_review"],
            bifa,
        )
        self.assertIs(
            persist_brand.call_args_list[1].kwargs["brand_review"],
            delisky,
        )
        for call in persist_brand.call_args_list:
            self.assertEqual(
    ...
    def test_rejects_review_without_brand_partitions(self):
    ...
    def test_rejects_unsaved_source_upload(self):
        ...
        with self.assertRaisesMessage(
            ...
            persist_raw_items_detail_review(
                ...
                review=self._review([SimpleNamespace(brand_code="BIFA")]),
                uploaded_by=self.user,
                reviewer=self.user,
            )
